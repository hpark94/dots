...
Never do more than the task asks. If you think of something extra, say it; do not
build it.
...
Spawn a NEW subagent. It must not be the Builder. Give it exactly this, filled in:
...
- If the Reviewer found problems: send them to a Builder to fix, then run Step 4
  again. On that re-run, tell the Reviewer to check only the fix and that nothing
  else broke, not the whole change again.
