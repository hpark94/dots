...
- [x] After a switch, the next `fzf` in an already-open shell shows the new
      Theme Mode's colors, in both zsh and bash.
- [x] After a switch, the next `bat` invocation in an already-open shell uses
      the new Theme Mode, in both zsh and bash.
- [x] The update happens at the next prompt with no manual trigger.
- [x] It works for shells inside a tmux pane and for a bare terminal outside
      tmux.
- [x] direnv, mise, and zoxide still fire on every prompt after the change; the
      pure prompt still works in zsh.
- [x] Nothing is signalled or killed by a switch; non-interactive shells and
      subshells are unaffected (the hook lives only in the interactive inits).
- [x] A shell opened before the first switch is still themed correctly at
      startup, and a brand-new pane/window starts in the current Theme Mode.
- [x] The startup-source line is replaced by one shared `_theme_reload`
      definition; only the shell-env fragment is re-sourced, never the full
      init.
- [x] Verified by manual smoke (switch in a live shell, confirm fzf/bat re-theme
