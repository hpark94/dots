...
the generated `shell-env.sh` fragment *before each command*: zsh via a `preexec`
...
`precmd`/`PROMPT_COMMAND` hook fires *after* the just-accepted command, so the
...
prompt line, since `line-init` fired before the switch; closing that would need a
per-keystroke hook, which is out of proportion to the gain.
...
what it saves; a gate that forks `stat` every command costs more than the work it
